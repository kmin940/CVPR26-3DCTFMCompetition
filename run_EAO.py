...
class AllAttentionPoolingHeads(nn.Module):
    """Multiple attention pooling heads with different learning rates"""
    def __init__(self, embed_dim, query_num, num_classes, num_heads, dropout, num_layers, ffn_mult, lrs):
        super().__init__()
        self.heads = nn.ModuleDict()
        self.param_groups = []
        for lr in lrs:
            name = f"head_lr_{lr:.0e}".replace("-", "_").replace(".", "_")
            ...
            self.param_groups.append({"params": head.parameters(), "lr": lr})

    def forward(self, x):
        """
        Args:
            x: spatial features [B, D, H, W, L] or [B, D, H*W*L]
        Returns:
            dict of {head_name: logits [B, num_classes]}
        """
...
def train_one_epoch(model, loader, opt, crit, device):
    ...
    tot_samples = 0
    per_head_losses = {name: 0.0 for name in model.heads.keys()}

    for xb, yb in loader:  # xb: (B, D, H, W, L), yb: (B,)
        batch_size = xb.size(0)
        xb, yb = xb.to(device, non_blocking=True), yb.to(device, non_blocking=True)

        out = model(xb)  # {head_name: logits}
        loss_heads = [crit(o, yb) for o in out.values()]
        per_head_losses = {name: per_head_losses[name] + l.item() * batch_size
                          for name, l in zip(model.heads.keys(), loss_heads)}
    ...
    # compute per-head average loss
    for name in per_head_losses.keys():
        per_head_losses[name] /= tot_samples

    return tot_loss / tot_samples, per_head_losses
...
def evaluate(model, loader, crit, device, num_classes, monitor_metric):
    ...
    for xb, yb in loader:
        ...
        outputs = model(xb)  # {head_name: logits}
        batch_size = xb.size(0)
    ...
    # pick best head by monitor metric
    best_name = max(names, key=lambda n: computed[n][monitor_metric])
    best_score = computed[best_name][monitor_metric]

    return best_name, best_score, computed


def save_single_head(model, name, path, results):
    sd = {f"heads.{name}."+k: v for k, v in model.heads[name].state_dict().items()}
    torch.save({"state_dict": sd, "head_name": name, "val_metrics": results}, path)


def extract_labels_from_dataset(ds: SpatialFeaturesDataset):
    labels = []
    for p in ds.paths:
...
def compute_sample_weights(ds: SpatialFeaturesDataset):
...
def main():
    ...
    ap.add_argument("--lrs", type=float, nargs="+",
                    default=[1e-5, 2e-5, 5e-5, 1e-4, 2e-4, 5e-4, 1e-3, 2e-3, 5e-3],
    ...
    ap.add_argument("--wandb_project", type=str, default="attn_pool_cvpr26")
    ...
    if use_wandb:
        wandb_dir = os.path.join(args.embeds_root, args.target, "wandb")
    ...
    ds_tr = SpatialFeaturesDataset(embeds_dir, labels_csv, split='train', target_column=args.target)

    # Get feature dimensions from first sample
    first_sample = ds_tr[0][0]
    print(f'First sample shape: {first_sample.shape}')

    # Determine embed_dim from the features
    ...
    if torch.isclose(sample_weights.max(), sample_weights.min()):
        ...
        sampler = RandomSampler(ds_tr, replacement=True, num_samples=len(ds_tr))
    else:
    ...
    ds_va = SpatialFeaturesDataset(embeds_dir, labels_csv, split='val', target_column=args.target)
    dl_va = DataLoader(ds_va, batch_size=args.batch_size,
                       shuffle=False, num_workers=args.num_workers, pin_memory=True)

    # Create model with multiple attention pooling heads
    lrs = args.lrs
    model = AllAttentionPoolingHeads(
        embed_dim=embed_dim,
    ...
    print(f'Created model with {len(model.heads)} attention pooling heads')
    print(f'Hyperparameters: query_num={args.query_num}, num_heads={args.num_heads}, '
          f'num_layers={args.num_layers}, dropout={args.dropout}, ffn_mult={args.ffn_mult}')
    ...
    best_score_per_head = {name: init_met_val for name in model.heads}
    best_met_per_head = {name: None for name in model.heads}
    ...
    for ep in range(1, args.epochs + 1):
        tr_loss, per_head_losses = train_one_epoch(model, dl_tr, opt, crit, device)
        ...
        if use_wandb:
            ...
            for name in head_names:
                log_dict[f"train/{name}/loss"] = per_head_losses[name]
        ...
        for name in head_names:
            train_losses_over_epochs[name].append(per_head_losses[name])
            ...
            if improved:
                ...
                best_met_per_head[name] = dict(all_met[name])
        ...
        if ep % args.log_interval == 0 or ep == args.epochs:
            ...
            for i, name in enumerate(head_names):
                ...
                axs[i].set_yticks([round(x, 1) for x in np.arange(0, 2.1, 0.1)])
            plt.tight_layout()
            plt.savefig(os.path.join(out_dir, f"progress.png"))
            plt.close(fig)

        # save best overall head and prune checkpoints
        if monitor_metric_mode == 'max' and best_met > best_met_overall:
            ...
            save_single_head(model, best_name, os.path.join(out_dir,
                f"best_overall_{monitor_metric}{best_met:.4f}_{best_name}_ep{ep}.pth"), results_best)
            ...
            results_best = all_met[best_name]
            save_single_head(model, best_name, os.path.join(out_dir,
                f"best_{monitor_metric}{best_met:.4f}_{best_name}_ep{ep}.pth"), results_best)
    ...
    # final summary
    print("Overall best attention pooling head:", {
